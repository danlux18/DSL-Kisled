...
Action *make_action(char *var, int signal, int howMany, char *value) {
...
static void emit_do_action() {
  emit("void do_action(int actuator, int act_signal, int value, boolean guard) {\n");
  emit("  if (!guard) return;\n");
  emit("  if (act_signal == OFF || act_signal == ON) {\n");
  emit("    digitalWrite(actuator, act_signal);\n");
  emit("  } else {\n");
  emit("    tone(actuator, value);\n");
  emit("    delay(act_signal);\n");
  emit("    noTone(actuator);\n");
  emit("  }\n");
  emit("}\n\n");
}
...
static void emit_actions(Action *list) {
  for (Action *p = list; p; p = p->next) {
    for (int i = 0; i < p->howMany; i++) {
      emit("  do_action(%s, %s, %s, act_guard);\n", p->var_name, act_sigs[p->sig_value], p->value);
      if (i != p->howMany - 1)
        emit("  delay(SHORT);\n");
    }
  }
